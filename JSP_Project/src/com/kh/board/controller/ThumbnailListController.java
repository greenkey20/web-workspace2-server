...
public class ThumbnailListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<Board> list = new BoardService().selectThumbnailList(); // 페이징 처리하려면 현재 페이지 정보를 요청 페이지로부터 받아야 함 vs 여기서는 dummy data가 많지 않고, 내가 업로드한 사진들만 보여줄 것이므로, 페이징 처리 생략 -> db에 전달할 값 없음 
